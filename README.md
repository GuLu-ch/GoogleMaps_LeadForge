...
- 支持从某个 Google Maps 采集任务创建“官网探索”二次任务，后续用于扩展官网联系方式和社媒信息。
...
官网探索模块在商家主记录中预留以下字段，当前已完成任务化入口、数据表和结果展示；真实网页访问、正则提取和浏览器兜底会在后续模块接入：
...
13. 任务完成后在“结果管理”页查看基础字段和官网探索字段，并导出 CSV 或 Excel。
...
官网探索页用于管理二次采集任务。当前已完成基础任务管理入口：

- 从已有 Google Maps 任务批次选择来源。
- 创建官网探索批次。
- 有官网商家进入待执行队列。
- 无官网商家直接标记为跳过。
- 展示探索批次、来源任务、完成数量、失败数量、跳过数量和探索任务列表。

后续模块将在该页面继续接入 Requests 静态采集、正则提取、浏览器自动化兜底、暂停/继续/停止和探索结果导出。
...
当前官网探索模块已完成任务化基础：从指定 Google Maps 批次读取命中商家，按商家去重后创建官网探索任务；无官网商家自动跳过。真实官网访问和信息提取在后续模块中开发。
...
默认清理脚本会删除 `data/app.sqlite3`、日志、导出、调试输出和截图等运行产物，但不会删除 `keyword.txt`、配置文件或 `drivers/selenium-cache/` 浏览器登录缓存。
